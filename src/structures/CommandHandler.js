...
class AvonCommands extends EventEmitter {
    ...
    async run(message){
        ...
        if(message.content === `<@${this.client.user.id}>`)
        {
            ...
            let b3 = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(`Vote`).setURL(`https://dsc.gg/codexdev`);
            let ro = new ActionRowBuilder().addComponents(b1,b2,b3);
            let embed = new EmbedBuilder().setColor(this.client.config.color).setFooter({text : `Made By CodeX Development ❤️` , iconURL : message.author.displayAvatarURL({dynamic : true})}).setDescription(
        ...
        try{
        let np = ['870179991462236170','1060027928206917682'];
        ...
        if(avonCommand.vote)
        {
            ...
            if(!voted && !this.client.config.owners.includes(message.author.id)){
                return message.channel.send({embeds : [new EmbedBuilder().setColor(config.color).setDescription(`${this.client.emoji.tick} | [Vote](https://dsc.gg/codexdev) Required Click [here](https://dsc.gg/codexdev)`)],components : [new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(`Vote`).setURL(`https://dsc.gg/codexdev`))]})
            }
        }
