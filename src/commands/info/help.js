...
class Help extends AvonCommand{
    ...
    async run(client,message,args,prefix){
        try{
        let em = new EmbedBuilder().setColor(client.config.color).setAuthor({name : `${client.user.username} Help Pannel`,iconURL : client.user.displayAvatarURL()}).setDescription(
            `Hey ${message.author} I am ${client.user.username}
             ...
             ${client.emoji.arrow} [Invite](https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands) | [Support](${client.config.server}) | [Vote](https://dsc.gg/codexdev)`
        ).addFields({
        ...
        }).setFooter({text : `Made By CodeX Development` , iconURL : message.guild.iconURL({dynamic : true})}).setThumbnail(message.author.displayAvatarURL({dynamic : true}))
        ...
        ]).setThumbnail(message.author.displayAvatarURL({dynamic : true})).setFooter({text : `Made By CodeX Development ❤️` , iconURL : message.guild.iconURL({dynamic : true})})
